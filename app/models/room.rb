class Room < ApplicationRecord
  ...
  scope :search_by_area, -> (area) { where("LOWER(address) LIKE ?", "%#{area}%") }
  scope :search_by_keyword, -> (keyword) { where("LOWER(name) LIKE :keyword OR LOWER(introduction) LIKE :keyword", keyword: "%#{keyword}%") }
